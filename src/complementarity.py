...
O paper usa apenas medias de longo prazo e ignora a dinamica temporal. Para geracao
hibrida, o que importa e se as duas fontes se completam ao longo do ano: quando uma cai,
a outra sobe. Isto e medido pela correlacao entre os ciclos mensais (climatologia) de
vento e de radiacao em cada ponto.

Indice de complementaridade: kappa = (1 - r) / 2, com r = correlacao de Pearson entre as
12 medias mensais de vento e de solar. kappa fica em [0, 1]:
  kappa proximo de 1  -> fontes anti-correlacionadas (otimo para hibrido)
  kappa proximo de 0.5 -> fontes independentes
  kappa proximo de 0  -> fontes sobem e descem juntas (ruim para hibrido)
"""
import sys, os
...
def nasa_complementarity():
    ...
    for f in sorted((config.RAW / "nasa").glob("nasa_*.csv")):
        ...
        m = df["date"].dt.month
        mw = df.groupby(m)["WS10M"].mean()
        ms = df.groupby(m)["ALLSKY_SFC_SW_DWN"].mean()
...
def inmet_complementarity():
    ...
    for _, s in st.iterrows():
        ...
        mw = df.groupby(t_local.dt.month)["wind"].mean()
        daily = _daily_solar_kwh(df)
        daily.index = pd.to_datetime(daily.index)
        ms = daily.groupby(daily.index.month).mean()
        common = mw.index.intersection(ms.index)
        r, k = kappa_from_monthly(mw.loc[common].values, ms.loc[common].values)
